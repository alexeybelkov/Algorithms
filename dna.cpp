...
void replace_nucl(DNA_SEQ& DNA, int& i, int& j, char& c) {
	...
	for (int u = 0; u < DNA.segm[i].size(); u++) {
		...
		if (j <= S) {
			...
			//cout << ' ' << "THERE "  << DNA.gene[nucl_index] << endl;
...
void nucleo_count(DNA_SEQ& DNA, int& i, int& j1, int& j2) {
	...
	for (int u = 0; u < DNA.segm[i].size(); u++) {
		K[0]++;
		S[0] += DNA.segm[i][u].second - DNA.segm[i][u].first + 1;
		if (j1 <= S[0]) {
			K[1] = K[0];
			S[1] = S[0];
			for (int v = u + 1; v < DNA.segm[i].size(); v++) {
				K[1]++;
				S[1] += DNA.segm[i][v].second - DNA.segm[i][v].first + 1;
				if (j2 <= S[1]) {
					j2 -= S[1] - (DNA.segm[i][v].second - DNA.segm[i][u].first + 1);
					break;
				}
			}
			j1 -= S[0] - (DNA.segm[i][u].second - DNA.segm[i][u].first + 1);
			int k = abs(K[1] - K[0] + 1);
			...
			for (int w = u; w < u + k; w++) {
				...
				if (k > 1) {
					...
					for (int v = DNA.segm[i][w].first + j1; v <= DNA.segm[i][w].first + j2; v++) {
						M[DNA.gene[v]] += 1;
					}
				}
			}
	...
	if (j1 > 0) {
		for (int u = 0; u < DNA.segm[i1 - 1].size(); u++) {
			...
			if (j1 - 1 <= S) {
				...
				for (int w = 0; w <= u; w++) {
					if (w < u) {
						if (DNA.segm[i1 - 1][w].first == 1 and DNA.segm[i1 - 1][w].second == 0) cout << "THERE";
						DNA.segm[DNA.n - 1].push_back(make_pair(DNA.segm[i1 - 1][w].first,DNA.segm[i1 - 1][w].second));
					}
					else {
						if (DNA.segm[i1 - 1][w].first == 1 and DNA.segm[i1 - 1][w].first + j1 - 1 == 0) cout << "THERE";
		...
		for (int u = 0; u < DNA.segm[i2 - 1].size(); u++) {
			...
			if (j2 <= S) {
				...
				for (int w = u; w < DNA.segm[i2 - 1].size(); w++) {
					if (w > u) {
						if (DNA.segm[i2 - 1][w].first == 1 and DNA.segm[i2 - 1][w].second == 0) cout << "THERE";
						DNA.segm[DNA.n - 1].push_back(make_pair(DNA.segm[i2 - 1][w].first,DNA.segm[i2 - 1][w].second));
					}
					else {
						if (DNA.segm[i2 - 1][w].first == 1 and DNA.segm[i2 - 1][w].first + j2 == 0) cout << "THERE";
